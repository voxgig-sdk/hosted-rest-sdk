...
type AppUserSessionEntity struct {
	name    string
...
func NewAppUserSessionEntity(client *core.HostedRestSDK, entopts map[string]any) *AppUserSessionEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *AppUserSessionEntity) GetName() string { return e.name }
...
func (e *AppUserSessionEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *AppUserSessionEntity) Deleted() bool {
	return e.deleted
}


func (e *AppUserSessionEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewAppUserSessionEntity(e.client, opts)
}

func (e *AppUserSessionEntity) Data(args ...any) any {
...
func (e *AppUserSessionEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an AppUserSession; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *AppUserSessionEntity) DataTyped(data ...AppUserSession) AppUserSession {
	if len(data) > 0 {
		return typedFrom[AppUserSession](e.Data(asMap(data[0])))
	}
	return typedFrom[AppUserSession](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through AppUserSession (all fields
// optional at the wire level).
func (e *AppUserSessionEntity) MatchTyped(match ...AppUserSession) AppUserSession {
	if len(match) > 0 {
		return typedFrom[AppUserSession](e.Match(asMap(match[0])))
	}
	return typedFrom[AppUserSession](e.Match())
}
...
func (e *AppUserSessionEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *AppUserSessionEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// AppUserSessionLoadMatch and returns an AppUserSession. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *AppUserSessionEntity) LoadTyped(reqmatch AppUserSessionLoadMatch, ctrl map[string]any) (AppUserSession, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return AppUserSession{}, err
	}
	return typedFrom[AppUserSession](res), nil
}



func (e *AppUserSessionEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *AppUserSessionEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *AppUserSessionEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *AppUserSessionEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *AppUserSessionEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
