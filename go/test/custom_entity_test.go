...
func TestCustomEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HOSTEDREST_TEST_CUSTOM_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		customRef01Data = core.ToMapAny(customRef01DataResult)
...
func customBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HOSTEDREST_TEST_CUSTOM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HOSTEDREST_TEST_CUSTOM_ENTID"])
