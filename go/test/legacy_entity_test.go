...
func TestLegacyEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HOSTEDREST_TEST_LEGACY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func legacyBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HOSTEDREST_TEST_LEGACY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HOSTEDREST_TEST_LEGACY_ENTID"])
