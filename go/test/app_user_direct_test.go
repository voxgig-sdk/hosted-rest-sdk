...
func TestAppUserDirect(t *testing.T) {
	t.Run("direct-list-app_user", func(t *testing.T) {
		setup := app_userDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-app_user", _mode); _shouldSkip {
			if _reason == "" {
		...
		result, err := client.Direct(map[string]any{
			"path":   "api/projects/{project_id}/app-users",
			"method": "GET",
			"params": params,
		})
		if setup.live {
	...
	t.Run("direct-load-app_user", func(t *testing.T) {
		setup := app_userDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-app_user", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type app_userDirectSetupResult struct {
	client *sdk.HostedRestSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func app_userDirectSetup(mockres any) *app_userDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"HOSTED_REST_TEST_APP_USER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HOSTED_REST_TEST_APP_USER_ENTID"]; ok {
		...
		return &app_userDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &app_userDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
