...
describe("CustomEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HOSTED_REST_TEST_CUSTOM_ENTID JSON to run live")
      return
    end
    ...
    local custom_ref01_data = helpers.to_map(vs.getprop(
      vs.getpath(setup.data, "new.custom"), "custom_ref01"))
    custom_ref01_data["path"] = setup.idmap["path01"]

    local custom_ref01_data_result, err = custom_ref01_ent:create(custom_ref01_data, nil)
    assert.is_nil(err)
...
function custom_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HOSTED_REST_TEST_CUSTOM_ENTID")
