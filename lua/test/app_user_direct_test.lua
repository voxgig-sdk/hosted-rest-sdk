...
describe("AppUserDirect", function()
  it("should direct-list-app_user", function()
    local setup = app_user_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-app_user", setup.live and "live" or "unit")
  ...
  it("should direct-load-app_user", function()
    local setup = app_user_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-app_user", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function app_user_direct_setup(mockres)
