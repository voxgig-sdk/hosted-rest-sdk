...
The Lua SDK for the HostedRest API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:AgentHealth(nil):load({ id = "example_id" }, nil)
...
local result, err = client:HostedRest(nil):load(
  { id = "test01" }, nil
)
