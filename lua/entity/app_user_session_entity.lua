-- HostedRest SDK AppUserSession entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AppUserSessionEntity = {}
AppUserSessionEntity.__index = AppUserSessionEntity


function AppUserSessionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AppUserSessionEntity)
  self._name = "app_user_session"
  self._client = client
...
function AppUserSessionEntity:get_name()
  return self._name
end


function AppUserSessionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AppUserSessionEntity.new(self._client, opts)
end
...
function AppUserSessionEntity:mark_deleted()
  self._deleted = true
end


function AppUserSessionEntity:deleted()
  return true == self._deleted
end


function AppUserSessionEntity:data_set(args)
  if args ~= nil then
...
function AppUserSessionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AppUserSessionEntity:match_set(args)
  if args ~= nil then
...
function AppUserSessionEntity:match_get()
...
function AppUserSessionEntity:stream(action, args, callopts)
...
function AppUserSessionEntity:_run_op(ctx, post_done)
  local utility = self._utility
