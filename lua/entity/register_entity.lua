-- HostedRest SDK Register entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RegisterEntity = {}
RegisterEntity.__index = RegisterEntity


function RegisterEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RegisterEntity)
  self._name = "register"
  self._client = client
...
function RegisterEntity:get_name()
  return self._name
end


function RegisterEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RegisterEntity.new(self._client, opts)
end
...
function RegisterEntity:mark_deleted()
  self._deleted = true
end


function RegisterEntity:deleted()
  return true == self._deleted
end


function RegisterEntity:data_set(args)
  if args ~= nil then
...
function RegisterEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RegisterEntity:match_set(args)
  if args ~= nil then
...
function RegisterEntity:match_get()
...
function RegisterEntity:stream(action, args, callopts)
...
---@param reqdata RegisterCreateData
---@param ctrl? table
---@return Register
---@return string? err
function RegisterEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function RegisterEntity:_run_op(ctx, post_done)
  local utility = self._utility
