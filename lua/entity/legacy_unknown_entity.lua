-- HostedRest SDK LegacyUnknown entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LegacyUnknownEntity = {}
LegacyUnknownEntity.__index = LegacyUnknownEntity


function LegacyUnknownEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LegacyUnknownEntity)
  self._name = "legacy_unknown"
  self._client = client
...
function LegacyUnknownEntity:get_name()
  return self._name
end


function LegacyUnknownEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LegacyUnknownEntity.new(self._client, opts)
end
...
function LegacyUnknownEntity:mark_deleted()
  self._deleted = true
end


function LegacyUnknownEntity:deleted()
  return true == self._deleted
end


function LegacyUnknownEntity:data_set(args)
  if args ~= nil then
...
function LegacyUnknownEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LegacyUnknownEntity:match_set(args)
  if args ~= nil then
...
function LegacyUnknownEntity:match_get()
...
function LegacyUnknownEntity:stream(action, args, callopts)
...
function LegacyUnknownEntity:_run_op(ctx, post_done)
  local utility = self._utility
