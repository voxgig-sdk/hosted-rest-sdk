-- HostedRest SDK CollectionRecord entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CollectionRecordEntity = {}
CollectionRecordEntity.__index = CollectionRecordEntity


function CollectionRecordEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CollectionRecordEntity)
  self._name = "collection_record"
  self._client = client
...
function CollectionRecordEntity:get_name()
  return self._name
end


function CollectionRecordEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CollectionRecordEntity.new(self._client, opts)
end
...
function CollectionRecordEntity:mark_deleted()
  self._deleted = true
end


function CollectionRecordEntity:deleted()
  return true == self._deleted
end


function CollectionRecordEntity:data_set(args)
  if args ~= nil then
...
function CollectionRecordEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CollectionRecordEntity:match_set(args)
  if args ~= nil then
...
function CollectionRecordEntity:match_get()
...
function CollectionRecordEntity:stream(action, args, callopts)
...
function CollectionRecordEntity:_run_op(ctx, post_done)
  local utility = self._utility
