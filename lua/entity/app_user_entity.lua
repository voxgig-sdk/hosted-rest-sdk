...
local AppUserEntity = {}
AppUserEntity.__index = AppUserEntity


function AppUserEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AppUserEntity)
  self._name = "app_user"
  self._client = client
...
function AppUserEntity:get_name()
  return self._name
end


function AppUserEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AppUserEntity.new(self._client, opts)
end
...
function AppUserEntity:mark_deleted()
  self._deleted = true
end


function AppUserEntity:deleted()
  return true == self._deleted
end


function AppUserEntity:data_set(args)
  if args ~= nil then
...
function AppUserEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AppUserEntity:match_set(args)
  if args ~= nil then
...
function AppUserEntity:match_get()
...
function AppUserEntity:stream(action, args, callopts)
...
function AppUserEntity:_run_op(ctx, post_done)
  local utility = self._utility
