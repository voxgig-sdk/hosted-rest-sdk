-- HostedRest SDK AppUserTotal entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AppUserTotalEntity = {}
AppUserTotalEntity.__index = AppUserTotalEntity


function AppUserTotalEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AppUserTotalEntity)
  self._name = "app_user_total"
  self._client = client
...
function AppUserTotalEntity:get_name()
  return self._name
end


function AppUserTotalEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AppUserTotalEntity.new(self._client, opts)
end
...
function AppUserTotalEntity:mark_deleted()
  self._deleted = true
end


function AppUserTotalEntity:deleted()
  return true == self._deleted
end


function AppUserTotalEntity:data_set(args)
  if args ~= nil then
...
function AppUserTotalEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AppUserTotalEntity:match_set(args)
  if args ~= nil then
...
function AppUserTotalEntity:match_get()
...
function AppUserTotalEntity:stream(action, args, callopts)
...
function AppUserTotalEntity:_run_op(ctx, post_done)
  local utility = self._utility
