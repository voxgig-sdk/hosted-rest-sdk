-- HostedRest SDK CollectionRecordList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local CollectionRecordListEntity = {}
CollectionRecordListEntity.__index = CollectionRecordListEntity


function CollectionRecordListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CollectionRecordListEntity)
  self._name = "collection_record_list"
  self._client = client
...
function CollectionRecordListEntity:get_name()
  return self._name
end


function CollectionRecordListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CollectionRecordListEntity.new(self._client, opts)
end
...
function CollectionRecordListEntity:mark_deleted()
  self._deleted = true
end


function CollectionRecordListEntity:deleted()
  return true == self._deleted
end


function CollectionRecordListEntity:data_set(args)
  if args ~= nil then
...
function CollectionRecordListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CollectionRecordListEntity:match_set(args)
  if args ~= nil then
...
function CollectionRecordListEntity:match_get()
...
function CollectionRecordListEntity:stream(action, args, callopts)
...
function CollectionRecordListEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return CollectionRecordListEntity
