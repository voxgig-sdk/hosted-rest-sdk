-- HostedRest SDK LegacyUnknownList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LegacyUnknownListEntity = {}
LegacyUnknownListEntity.__index = LegacyUnknownListEntity


function LegacyUnknownListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LegacyUnknownListEntity)
  self._name = "legacy_unknown_list"
  self._client = client
...
function LegacyUnknownListEntity:get_name()
  return self._name
end


function LegacyUnknownListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LegacyUnknownListEntity.new(self._client, opts)
end
...
function LegacyUnknownListEntity:mark_deleted()
  self._deleted = true
end


function LegacyUnknownListEntity:deleted()
  return true == self._deleted
end


function LegacyUnknownListEntity:data_set(args)
  if args ~= nil then
...
function LegacyUnknownListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LegacyUnknownListEntity:match_set(args)
  if args ~= nil then
...
function LegacyUnknownListEntity:match_get()
...
function LegacyUnknownListEntity:stream(action, args, callopts)
...
function LegacyUnknownListEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return LegacyUnknownListEntity
