-- HostedRest SDK AppUserLogin entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AppUserLoginEntity = {}
AppUserLoginEntity.__index = AppUserLoginEntity


function AppUserLoginEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AppUserLoginEntity)
  self._name = "app_user_login"
  self._client = client
...
function AppUserLoginEntity:get_name()
  return self._name
end


function AppUserLoginEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AppUserLoginEntity.new(self._client, opts)
end
...
function AppUserLoginEntity:mark_deleted()
  self._deleted = true
end


function AppUserLoginEntity:deleted()
  return true == self._deleted
end


function AppUserLoginEntity:data_set(args)
  if args ~= nil then
...
function AppUserLoginEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AppUserLoginEntity:match_set(args)
  if args ~= nil then
...
function AppUserLoginEntity:match_get()
...
function AppUserLoginEntity:stream(action, args, callopts)
...
function AppUserLoginEntity:_run_op(ctx, post_done)
  local utility = self._utility
