-- HostedRest SDK AgentUserList entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AgentUserListEntity = {}
AgentUserListEntity.__index = AgentUserListEntity


function AgentUserListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AgentUserListEntity)
  self._name = "agent_user_list"
  self._client = client
...
function AgentUserListEntity:get_name()
  return self._name
end


function AgentUserListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AgentUserListEntity.new(self._client, opts)
end
...
function AgentUserListEntity:mark_deleted()
  self._deleted = true
end


function AgentUserListEntity:deleted()
  return true == self._deleted
end


function AgentUserListEntity:data_set(args)
  if args ~= nil then
...
function AgentUserListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AgentUserListEntity:match_set(args)
  if args ~= nil then
...
function AgentUserListEntity:match_get()
...
function AgentUserListEntity:stream(action, args, callopts)
...
function AgentUserListEntity:_run_op(ctx, post_done)
  local utility = self._utility
