-- HostedRest SDK LegacyUser entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local LegacyUserEntity = {}
LegacyUserEntity.__index = LegacyUserEntity


function LegacyUserEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LegacyUserEntity)
  self._name = "legacy_user"
  self._client = client
...
function LegacyUserEntity:get_name()
  return self._name
end


function LegacyUserEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LegacyUserEntity.new(self._client, opts)
end
...
function LegacyUserEntity:mark_deleted()
  self._deleted = true
end


function LegacyUserEntity:deleted()
  return true == self._deleted
end


function LegacyUserEntity:data_set(args)
  if args ~= nil then
...
function LegacyUserEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LegacyUserEntity:match_set(args)
  if args ~= nil then
...
function LegacyUserEntity:match_get()
...
function LegacyUserEntity:stream(action, args, callopts)
...
---@param reqmatch LegacyUserLoadMatch
---@param ctrl? table
---@return LegacyUser
---@return string? err
function LegacyUserEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function LegacyUserEntity:_run_op(ctx, post_done)
  local utility = self._utility
