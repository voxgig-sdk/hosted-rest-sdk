...
class FtCtrl extends HostedRestControl
{
...
class FtClient
{
    ...
    private HostedRestUtility $utility;

    public function __construct(HostedRestUtility $utility, array $options)
    {
    ...
    public function get_utility(): HostedRestUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HostedRestContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HostedRestContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HostedRestContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HostedRestConfig::make_feature($name);
    ...
    private static function buildUrl(HostedRestSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HostedRestError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HostedRestResponse($fetched);
        }
    ...
    private function fail(HostedRestContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HostedRestContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
