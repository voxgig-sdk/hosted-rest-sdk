...
module HostedRestUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HostedRestResponse.new({})
    ...
    if fetch_err
      ...
      response = HostedRestResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HostedRestResponse.new(fetched)
    else
