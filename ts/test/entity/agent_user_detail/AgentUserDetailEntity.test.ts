...
describe('AgentUserDetailEntity', async () => {
  ...
  test('instance', async () => {
    ...
    const ent = testsdk.AgentUserDetail()
    assert(null != ent)
  })
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'agent_user_detail.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/agent_user_detail/AgentUserDetailTestData.json')
  ...
  const idmapEnvVal = process.env['HOSTED_REST_TEST_AGENT_USER_DETAIL_ENTID']
  ...
  idmap = env['HOSTED_REST_TEST_AGENT_USER_DETAIL_ENTID']
