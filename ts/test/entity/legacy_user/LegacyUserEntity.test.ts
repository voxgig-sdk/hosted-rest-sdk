...
describe('LegacyUserEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'legacy_user.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set HOSTED_REST_TEST_LEGACY_USER_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/legacy_user/LegacyUserTestData.json')
  ...
  const idmapEnvVal = process.env['HOSTED_REST_TEST_LEGACY_USER_ENTID']
  ...
  idmap = env['HOSTED_REST_TEST_LEGACY_USER_ENTID']
