...
describe('AppUserDirect', async () => {
  ...
  test('direct-load-app_user', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-app_user', setup.live)) return
  ...
  test('direct-list-app_user', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-app_user', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HOSTED_REST_TEST_APP_USER_ENTID']
