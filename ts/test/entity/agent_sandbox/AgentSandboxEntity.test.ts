...
describe('AgentSandboxEntity', async () => {
  ...
  test('instance', async () => {
    ...
    const ent = testsdk.AgentSandbox()
    assert(null != ent)
  })
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'load']) {
      if (maybeSkipControl(t, 'entityOp', 'agent_sandbox.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/agent_sandbox/AgentSandboxTestData.json')
  ...
  const idmapEnvVal = process.env['HOSTED_REST_TEST_AGENT_SANDBOX_ENTID']
