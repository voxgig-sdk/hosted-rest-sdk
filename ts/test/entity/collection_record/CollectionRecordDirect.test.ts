...
describe('CollectionRecordDirect', async () => {
  ...
  test('direct-load-collection_record', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-collection_record', setup.live)) return
    if (skipIfMissingIds(t, setup, ["collection_id01","id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HOSTED_REST_TEST_COLLECTION_RECORD_ENTID']
