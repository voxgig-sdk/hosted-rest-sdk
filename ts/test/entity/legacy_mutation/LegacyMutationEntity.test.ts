...
describe('LegacyMutationEntity', async () => {
  ...
  test('instance', async () => {
    ...
    const ent = testsdk.LegacyMutation()
    assert(null != ent)
  })
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['create', 'update']) {
      if (maybeSkipControl(t, 'entityOp', 'legacy_mutation.' + op, live)) return
    }
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/legacy_mutation/LegacyMutationTestData.json')
  ...
  const idmapEnvVal = process.env['HOSTED_REST_TEST_LEGACY_MUTATION_ENTID']
  ...
  idmap = env['HOSTED_REST_TEST_LEGACY_MUTATION_ENTID']
