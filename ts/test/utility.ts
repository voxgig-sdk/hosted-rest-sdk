...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.HOSTED_REST_TEST_LIVE ||
    'TRUE' === process.env.HOSTED_REST_TEST_OVERRIDE
  ) {
  ...
  m.HOSTED_REST_TEST_EXPLAIN = process.env.HOSTED_REST_TEST_EXPLAIN || m.HOSTED_REST_TEST_EXPLAIN
