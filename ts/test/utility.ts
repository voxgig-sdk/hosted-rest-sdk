...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.HOSTEDREST_TEST_LIVE ||
    'TRUE' === process.env.HOSTEDREST_TEST_OVERRIDE
  ) {
  ...
  m.HOSTEDREST_TEST_EXPLAIN = process.env.HOSTEDREST_TEST_EXPLAIN || m.HOSTEDREST_TEST_EXPLAIN
