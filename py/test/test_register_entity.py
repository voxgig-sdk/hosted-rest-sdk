...
class TestRegisterEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Register(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _register_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "register." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOSTED_REST_TEST_REGISTER_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        register_ref01_ent = client.Register(None)
        register_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.register"), "register_ref01"))

        register_ref01_data = helpers.to_map(runner.entity_data(register_ref01_ent.create(register_ref01_data, None)))
        assert register_ref01_data is not None
        assert register_ref01_data["id"] is not None



def _register_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/register/RegisterTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "HOSTED_REST_TEST_REGISTER_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOSTED_REST_TEST_REGISTER_ENTID"))
