...
class TestLegacyUserDirect:

    def test_should_direct_load_legacy_user(self):
        setup = _legacy_user_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-legacy_user", "live" if setup["live"] else "unit")
        if _skip:
...
def _legacy_user_direct_setup(mockres):
