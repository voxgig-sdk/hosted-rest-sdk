...
class TestAgentUserDetailDirect:

    def test_should_direct_load_agent_user_detail(self):
        setup = _agent_user_detail_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-agent_user_detail", "live" if setup["live"] else "unit")
        if _skip:
...
def _agent_user_detail_direct_setup(mockres):
