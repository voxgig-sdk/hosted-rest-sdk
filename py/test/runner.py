...
class HostedRestTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HostedRestTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HostedRestTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HostedRestTestRunner.getenv("HOSTED_REST_TEST_LIVE")
        override = HostedRestTestRunner.getenv("HOSTED_REST_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HostedRestTestRunner.getenv(key)
        ...
        explain = HostedRestTestRunner.getenv("HOSTED_REST_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HOSTED_REST_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HostedRestTestRunner._test_control is not None:
            return HostedRestTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HostedRestTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HostedRestTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HostedRestTestRunner.load_test_control()
...
def load_env_local():
    HostedRestTestRunner.load_env_local()


def env_override(m):
    return HostedRestTestRunner.env_override(m)


def entity_data(v):
    return HostedRestTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HostedRestTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HostedRestTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HostedRestTestRunner.load_test_control()


def live_delay_ms():
    return HostedRestTestRunner.live_delay_ms()
