...
class TestAppUserDirect:

    def test_should_direct_list_app_user(self):
        setup = _app_user_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-app_user", "live" if setup["live"] else "unit")
        if _skip:
    ...
    def test_should_direct_load_app_user(self):
        setup = _app_user_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-app_user", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
...
def _app_user_direct_setup(mockres):
