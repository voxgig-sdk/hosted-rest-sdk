...
class TestAgentUserListDirect:

    def test_should_direct_list_agent_user_list(self):
        setup = _agent_user_list_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-agent_user_list", "live" if setup["live"] else "unit")
        if _skip:
...
def _agent_user_list_direct_setup(mockres):
