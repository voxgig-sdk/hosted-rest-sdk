...
class TestAgentUserDetailEntity:

    def test_should_create_instance(self):
        testsdk = HostedRestSDK.test(None, None)
        ent = testsdk.AgentUserDetail(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _agent_user_detail_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "agent_user_detail." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOSTED_REST_TEST_AGENT_USER_DETAIL_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        agent_user_detail_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.agent_user_detail")))
        agent_user_detail_ref01_data = None
        if len(agent_user_detail_ref01_data_raw) > 0:
        ...
        agent_user_detail_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(agent_user_detail_ref01_data_dt0_loaded))
        assert agent_user_detail_ref01_data_dt0_load_result is not None
        assert agent_user_detail_ref01_data_dt0_load_result["id"] == agent_user_detail_ref01_data["id"]



def _agent_user_detail_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/agent_user_detail/AgentUserDetailTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOSTED_REST_TEST_AGENT_USER_DETAIL_ENTID"))
