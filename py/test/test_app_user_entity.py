...
class TestAppUserEntity:

    def test_should_create_instance(self):
        testsdk = HostedRestSDK.test(None, None)
        ent = testsdk.AppUser(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.AppUser(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.AppUser(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _app_user_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "update", "load", "remove"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "app_user." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOSTED_REST_TEST_APP_USER_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        app_user_ref01_ent = client.AppUser(None)
        ...
        app_user_ref01_data = helpers.to_map(runner.entity_data(app_user_ref01_ent.create(app_user_ref01_data, None)))
        ...
        app_user_ref01_resdata_up0 = helpers.to_map(runner.entity_data(app_user_ref01_ent.update(app_user_ref01_data_up0_up, None)))
        ...
        app_user_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(app_user_ref01_data_dt0_loaded))
...
def _app_user_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/app_user/AppUserTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "HOSTED_REST_TEST_APP_USER_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOSTED_REST_TEST_APP_USER_ENTID"))
