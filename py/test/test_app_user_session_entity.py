...
class TestAppUserSessionEntity:

    def test_should_create_instance(self):
        testsdk = HostedRestSDK.test(None, None)
        ent = testsdk.AppUserSession(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _app_user_session_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "app_user_session." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOSTED_REST_TEST_APP_USER_SESSION_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        app_user_session_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.app_user_session")))
        app_user_session_ref01_data = None
        if len(app_user_session_ref01_data_raw) > 0:
...
def _app_user_session_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/app_user_session/AppUserSessionTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "HOSTED_REST_TEST_APP_USER_SESSION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOSTED_REST_TEST_APP_USER_SESSION_ENTID"))
