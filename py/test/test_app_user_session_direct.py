...
class TestAppUserSessionDirect:

    def test_should_direct_load_app_user_session(self):
        setup = _app_user_session_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-app_user_session", "live" if setup["live"] else "unit")
        if _skip:
...
def _app_user_session_direct_setup(mockres):
