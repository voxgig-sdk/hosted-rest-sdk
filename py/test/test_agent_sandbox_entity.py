...
class TestAgentSandboxEntity:

    def test_should_create_instance(self):
        testsdk = HostedRestSDK.test(None, None)
        ent = testsdk.AgentSandbox(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _agent_sandbox_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "agent_sandbox." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HOSTED_REST_TEST_AGENT_SANDBOX_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        agent_sandbox_ref01_ent = client.AgentSandbox(None)
        agent_sandbox_ref01_data = helpers.to_map(vs.getprop(
            vs.getpath(setup["data"], "new.agent_sandbox"), "agent_sandbox_ref01"))

        agent_sandbox_ref01_data = helpers.to_map(runner.entity_data(agent_sandbox_ref01_ent.create(agent_sandbox_ref01_data, None)))
        assert agent_sandbox_ref01_data is not None

        # LOAD
        agent_sandbox_ref01_match_dt0 = {}
        agent_sandbox_ref01_data_dt0_loaded = agent_sandbox_ref01_ent.load(agent_sandbox_ref01_match_dt0, None)
        assert agent_sandbox_ref01_data_dt0_loaded is not None



def _agent_sandbox_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/agent_sandbox/AgentSandboxTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HOSTED_REST_TEST_AGENT_SANDBOX_ENTID"))
