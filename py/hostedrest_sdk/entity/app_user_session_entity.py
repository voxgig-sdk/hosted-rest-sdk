...
class AppUserSessionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return AppUserSessionEntity(self._client, opts)
    ...
    def data_get(self) -> AppUserSession:
    ...
    def match_get(self) -> AppUserSession:
    ...
    def load(self, reqmatch=None, ctrl=None) -> AppUserSession:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.AppUserSession().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
