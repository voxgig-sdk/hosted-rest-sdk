...
The Python SDK for the HostedRest API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.AgentHealth(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.HostedRest(None).load(
    {"id": "test01"}, None
)
