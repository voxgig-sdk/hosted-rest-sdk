// Command HostedRest-cli is a CLI + REPL for the HostedRest SDK,
...
const prompt = "hosted-rest"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "agent_health agent_sandbox agent_user_detail agent_user_list app_user app_user_login app_user_session app_user_total app_user_verify authentication collection collection_record collection_record_list custom legacy legacy_mutation legacy_unknown legacy_unknown_list legacy_user legacy_user_list login register"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HOSTED_REST_APIKEY (api key), HOSTED_REST_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HOSTED_REST_APIKEY carries the API key and
	// HOSTED_REST_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HOSTED_REST_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HOSTED_REST_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHostedRestSDK(opts)
