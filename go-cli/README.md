...
boru-driven command-line client **and** interactive REPL for the HostedRest
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hosted-rest-cli)
make build

# 2. See usage (words, entities, env vars)
./hosted-rest-cli --help

# 3. Provide credentials once, via the environment
export HOSTED_REST_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./hosted-rest-cli load 1 agent_health            # {id:1} shorthand
./hosted-rest-cli load '{id:1}' agent_health       # explicit match map

# 5. Override the API base URL for a single call
HOSTED_REST_BASE=https://api.example.com ./hosted-rest-cli load 1 agent_health

# 6. No arguments -> interactive REPL
./hosted-rest-cli
hosted-rest> load 1 agent_health
hosted-rest> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hosted-rest-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hosted-rest-cli load 1 agent_health
   ```
...
```sh
./hosted-rest-cli load 1 agent_health          # scalar shorthand for {id:1}
./hosted-rest-cli load '{id:1}' agent_health     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `hosted-rest>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `agent_health`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hosted-rest-cli`. |
...
(`./hosted-rest-cli <expr>`) and interactively (the REPL), and expressions compose the
