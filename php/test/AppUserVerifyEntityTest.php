...
class AppUserVerifyEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HOSTED_REST_TEST_APP_USER_VERIFY_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $app_user_verify_ref01_data = Helpers::to_map(is_object($app_user_verify_ref01_data_result) && method_exists($app_user_verify_ref01_data_result, 'data_get') ? $app_user_verify_ref01_data_result->data_get() : $app_user_verify_ref01_data_result);
...
function app_user_verify_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("HOSTED_REST_TEST_APP_USER_VERIFY_ENTID");
