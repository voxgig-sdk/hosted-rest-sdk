...
class AuthenticationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HOSTED_REST_TEST_AUTHENTICATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function authentication_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("HOSTED_REST_TEST_AUTHENTICATION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["HOSTED_REST_TEST_AUTHENTICATION_ENTID"]);
