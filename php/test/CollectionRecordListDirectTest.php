...
class CollectionRecordListDirectTest extends TestCase
{
    public function test_direct_list_collection_record_list(): void
    {
        $setup = collection_record_list_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-collection_record_list", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["slug01"] as $_liveKey) {
