...
[MCP](https://modelcontextprotocol.io) server exposing the HostedRest SDK as
two agent tools — `hosted-rest_list` and `hosted-rest_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hosted-rest-mcp)
make build

# 2. Provide credentials via the environment
export HOSTED_REST_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user hosted-rest \
  -- /absolute/path/to/hosted-rest-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hosted-rest-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hosted-rest_list: first page of records
{ "entity": "agent_user_list" }
{ "entity": "agent_user_list", "query": { } }

// hosted-rest_load: one record by id
{ "entity": "agent_health", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hosted-rest-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hosted-rest \
     -- "$PWD"/dist/*/hosted-rest-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hosted-rest_list` and `hosted-rest_load` tools now appear
   in new sessions. Ask the agent to *"list agent_user_list using hosted-rest"*
   and it calls `hosted-rest_list` with `{"entity":"agent_user_list"}`.
...
```sh
export HOSTED_REST_APIKEY=sk_live_xxx            # API key
export HOSTED_REST_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `hosted-rest_list` tool
...
### Call the `hosted-rest_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hosted-rest_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hosted-rest_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HOSTED_REST_APIKEY` | API key sent with every request. |
| `HOSTED_REST_BASE` | Optional override of the API base URL. |
...
```sh
./hosted-rest-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hosted-rest_load","arguments":{"entity":"agent_health","query":{"id":1}}}}'
```
