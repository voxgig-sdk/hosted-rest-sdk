...
class AppUserDirectTest < Minitest::Test
  def test_direct_list_app_user
    setup = app_user_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-app_user", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_app_user
    setup = app_user_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-app_user", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def app_user_direct_setup(mockres)
