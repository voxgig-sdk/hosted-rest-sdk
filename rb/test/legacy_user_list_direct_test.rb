...
class LegacyUserListDirectTest < Minitest::Test
  def test_direct_list_legacy_user_list
    setup = legacy_user_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-legacy_user_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def legacy_user_list_direct_setup(mockres)
