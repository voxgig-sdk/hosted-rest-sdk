...
class LegacyUnknownListDirectTest < Minitest::Test
  def test_direct_list_legacy_unknown_list
    setup = legacy_unknown_list_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-legacy_unknown_list", setup[:live] ? "live" : "unit")
    if _should_skip
...
def legacy_unknown_list_direct_setup(mockres)
