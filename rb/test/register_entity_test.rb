...
class RegisterEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set HOSTED_REST_TEST_REGISTER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    register_ref01_data = Helpers.to_map(register_ref01_data_result.respond_to?(:data_get) ? register_ref01_data_result.data_get : register_ref01_data_result)
...
def register_basic_setup(extra)
  ...
  entid_env_raw = ENV["HOSTED_REST_TEST_REGISTER_ENTID"]
